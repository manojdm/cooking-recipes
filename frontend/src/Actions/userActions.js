import { ADD_USER_DISPATCH, ADD_USER_FAIL, ADD_USER_SUCCESS, AUTH_USER_DISPATCH, AUTH_USER_FAIL, AUTH_USER_SUCCESS, DELETE_USER_DISPATCH, DELETE_USER_FAIL, DELETE_USER_SUCCESS, EDIT_USER_DISPATCH, EDIT_USER_FAIL, EDIT_USER_SUCCESS, FETCH_USERS_DISPATCH, FETCH_USERS_FAIL, FETCH_USERS_SUCCESS, FETCH_USER_DISPATCH, FETCH_USER_FAIL, FETCH_USER_SUCCESS } from "../constants/userConstants";
...
export const authLogin = (user) => async (dispatch) => {
    dispatch({type : AUTH_USER_DISPATCH , loading : true})

    try {
        const header = {headers : {'Content-Type' : 'application/json'}}
        const {data} = await axios.post(`http://localhost:5000/api/users/admin/login` , user , header);

        dispatch({type : AUTH_USER_SUCCESS , success : true , loading : false , payload : data});
