...
export const addComments = (id , comment) => async(dispatch) => {
    dispatch({type : ADD_COMMENT_DISPATCH , loading : true})

    try {
        const header = {headers : {'Content-Type' : 'application/json'}}
        const {data} = await axios.post(`http://localhost:5000/api/posts/comment/${id}` , comment , header);
        dispatch({type : ADD_COMMENT_SUCCESS , success : true , loading : true});
    } catch (error) {
